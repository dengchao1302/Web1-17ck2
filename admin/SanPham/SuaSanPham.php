...
if(isset($_POST["btnUpdate"]))
{
  ...
  $sql = "update SanPham set TenSanPham='" . $tenSanPham . "',GiaSanPham='" . $giaSanPham . "',SoLuongTon='" . $soLuongTon . "',MaLoaiSanPham='" . $maLoaiSanPham . "',MaHangSanXuat='" . $maHangSanXuat . "' where MaSanPham = '" . $_GET["id"] . "' ";
  ...
  <div id="wrapper">
    ...
    <div id="content-wrapper">
      ...
      <div class="container-fluid">
        ...
        $listHSX = DataProvider::execQuery($sql);
        ...
        <div class="card mb-3">
          ...
          <div class="card-body">
            <div class="table-responsive">
              <form action="SuaSanPham.php?id=<?php echo $_GET["id"] ?>" method="POST" name="frmSua" id="frmSua">
                <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                  ...
                  <thead>
                    ...
                    if ($check == 1) {
                      ?>
                      <tr>
                        <td colspan="2" style="background-color: green"> Cập nhật nhà sản xuất thành công </td>
                      </tr>
                      <?php 
                    } else if ($check == 0) {
                      ?>
                      <tr>
                        <td colspan="2" style="background-color: red"> Cập nhật nhà sản xuất thất bại </td>
                      </tr>
                      <?php 
                    }
                    ?>
                  ...
                  <tbody>
                    ...
                    while ($row = mysqli_fetch_array($list)) {
                      ...
                      <tr>
                      <th>Mã sản phẩm:</th>
                      <td><input type="text" name="MaSanPham" value="<?php echo $row["MaSanPham"]; ?>" disabled></td>
                    </tr>
                    <tr>
                      <th>Tên sản phẩm:</th>
                    ...
                    <tr>
                      <th>Số lượng tồn:</th>
                      <td><input type="number" name="SoLuongTon" value="<?php echo $row["SoLuongTon"]; ?>"></td>
                    </tr>
                    <tr>
                      <th>Loại sản phẩm:</th>
                      <td>
                        <select name="MaLoaiSanPham">
                          <?php
                          while ($rowLSP = mysqli_fetch_array($listLSP)) {
                            ?>
                            <option value="<?php echo $rowLSP["MaLoaiSanPham"]; ?>"
                              <?php
                                if ($row["MaLoaiSanPham"] == $rowLSP["MaLoaiSanPham"]) {
                                  echo "selected";
                                }
                              ?>
                              >
                              <?php echo $rowLSP["TenLoaiSanPham"]; ?>
                            </option>
                            <?php
                          }
                          ?>
                    ...
                    <tr>
                      ...
                      <td>
                        <select name="MaHangSanXuat">
                          <?php
                          while ($rowHSX = mysqli_fetch_array($listHSX)) {
                            ?>
                            <option value="<?php echo $rowHSX["MaHangSanXuat"]; ?>"
                              <?php
                                if ($row["MaHangSanXuat"] == $rowHSX["MaHangSanXuat"]) {
                                  echo "selected";
                                }
                              ?>
                              >
